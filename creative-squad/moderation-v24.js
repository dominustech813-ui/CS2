(()=>{
  ...
  async function avatarFor(p){try{return await url('avatars',p?.avatar_path)}catch{return '../creative-squad/logo-cdl.svg'}}
  ...
  async function render(term=''){
    ...
    const html=await Promise.all(rows.map(async x=>{const p=x.p,m=x.m,s=effectiveStatus(m),a=await avatarFor(p),isMain=String(p.email||'').toLowerCase()===ADMIN_EMAIL;let info='Ativo';if(s==='banned')info='Banido'+(m?.reason?' • '+m.reason:'');if(s==='suspended')info='Suspenso até '+new Date(m.suspended_until).toLocaleString('pt-BR')+(m?.reason?' • '+m.reason:'');return `<div class="modUser"><div class="modTop"><img class="modAvatar" src="${esc(a)}"><div><div class="modName">${window.CDLRoleChoice?CDLRoleChoice.nameHtml(p.id,p.display_name||p.nickname||'Membro CDL'):esc(p.display_name||p.nickname||'Membro CDL')}</div><div class="modEmail">${esc(p.email||'')}</div></div><span class="modStatus ${s}">${s==='banned'?'BANIDO':s==='suspended'?'SUSPENSO':'ATIVO'}</span></div><div class="modInfo">${esc(info)}</div>${isMain?'<div class="modInfo">👑 Administrador principal</div>':`<div class="modActions">${s!=='banned'?'<button class="modBtn modBan" onclick="CDLModeration.ban(\''+esc(p.id)+'\')">Banir</button>':''}${s!=='suspended'?'<button class="modBtn modSuspend" onclick="CDLModeration.suspend(\''+esc(p.id)+'\',1)">1 hora</button><button class="modBtn modSuspend" onclick="CDLModeration.suspend(\''+esc(p.id)+'\',24)">1 dia</button><button class="modBtn modSuspend" onclick="CDLModeration.suspend(\''+esc(p.id)+'\',168)">7 dias</button><button class="modBtn modSuspend" onclick="CDLModeration.suspend(\''+esc(p.id)+'\',720)">30 dias</button>':''}${s!=='active'?'<button class="modBtn modRestore" onclick="CDLModeration.restore(\''+esc(p.id)+'\')">Reativar</button>':''}<button class="modBtn modDelete" onclick="CDLModeration.deleteAccount('${esc(p.id)}','${esc(String(p.display_name||p.nickname||'Membro CDL').replace(/'/g,'&#39;'))}')">🗑️ Excluir conta</button></div>`}</div>`}));box.innerHTML=html.join('')
  }
  ...
  async function deleteAccount(uid,name){
    if(!admin())return;
    if(!confirm(`Excluir definitivamente a conta de ${name||'este participante'}?\n\nA sessão será encerrada em todos os aparelhos e a pessoa precisará criar outra conta CDL.`))return;
    try{
      const {data}=await sb.auth.getSession();const token=data?.session?.access_token;if(!token)return alert('Sua sessão de administrador expirou. Entre novamente.');
      const r=await fetch('https://yivgtvgyaalhlcxatbfc.supabase.co/functions/v1/cdl-account-auth',{method:'POST',headers:{'Content-Type':'application/json','apikey':SUPABASE_KEY,'Authorization':'Bearer '+token},body:JSON.stringify({action:'admin_delete_account',user_id:uid})});
      const j=await r.json().catch(()=>({}));
      if(!r.ok||!j.ok)return alert(j.error||'Não foi possível excluir a conta.');
      alert('Conta excluída. A sessão dessa pessoa será encerrada e ela precisará criar outra conta CDL.');
      window.__modRows=null;await caches();await load();
    }catch(e){alert('Não foi possível excluir a conta agora.')}
  }
  ...
  async function checkOwn(){
    ...
    location.replace('../?logout=1&blocked='+s+'&v=36')
  }
  ...
  window.CDLModeration={render,ban,suspend,restore,deleteAccount,load,start};
