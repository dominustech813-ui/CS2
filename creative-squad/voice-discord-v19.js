(()=>{
  ...
  async function avatarFor(p){
    try{return await url('avatars',p?.avatar_path)}catch{return '../creative-squad/logo-cdl.svg'}
  }
  ...
  function installDock(){
    ...
    dock.innerHTML=`
      ...
      <div class="dvSelf">
        <div class="dvAvatarWrap"><img id="voiceSelfAvatar" class="dvAvatar" src="../creative-squad/logo-cdl.svg" alt="Perfil"><span class="dvOnline"></span></div>
  ...
  function wrapCore(){
    try{const coreRender=renderVoices;renderVoices=function(){try{coreRender()}catch{}schedule()}}catch{}
    try{const coreJoin=joinVoice;joinVoice=async function(id){const out=await coreJoin(id);installDock();schedule();setTimeout(schedule,700);return out}}catch{}
    try{const coreLeave=leaveVoice;leaveVoice=async function(){const out=await coreLeave();schedule();return out}}catch{}
    try{const coreMute=toggleMute;toggleMute=function(){const out=coreMute();refreshSelf();schedule();return out}}catch{}
    try{const coreEnter=enter;enter=function(){const out=coreEnter();start();return out}}catch{}
  }
  ...
  async function start(){
    ...
    await paintVoices();
    uiSub=sb.channel('cdl-discord-voice-ui-'+Date.now()).on('postgres_changes',{event:'*',schema:'public',table:'voice_presence'},schedule).subscribe();
